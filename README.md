...
*日本語 | [English](README.en.md)* ・ バージョン **0.1.5**（SemVer）
...
- **ローカル完結・非公開** — `127.0.0.1` のみにバインド。使用量データは各環境の手元にだけ蓄積し、LAN にも外部にも出さない（月次パネルの取得のみ、Anthropic の API へ読み取りリクエストを送ります。→[月次クレジット枠](#月次クレジット枠extra-usage)）。
