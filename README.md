...
*日本語 | [English](README.en.md)* ・ バージョン **0.3.0**（SemVer）
...
- **used 線**: レート枠の使用率（%）。pace 乖離に応じて 青（遅れ）〜灰（標準どおり）〜赤（先行）に着色。**Codex のパネルだけは遅れ側が緑**（どちらのツールの枠かを一目で分けるため）。
- **even pace（点線）**: 標準消費ペース。5h は均等直線、7d は就業時間ベースの階段。
- **now 線**: 現在時刻（毎秒更新）。ホバーで各時点の used / even / pace 乖離を表示。

上から順に **5h 枠**（次の 5h リセットまでの5時間）、**7d 枠**（次の 7d リセットまでの7日間）。以降は環境に応じて増えます —— **月次クレジット枠（extra usage）が使えるアカウントでは `1mo`**（次の月初リセットまで。→[月次クレジット枠](#月次クレジット枠extra-usage)）、**Codex CLI を使っていればその枠**（`codex 5h` / `codex 7d` など。Codex が返す窓の構成にそのまま追従します。→[Codex のレート枠](#codex-のレート枠任意)）。

パネルごとに横軸の意味が違うので、**全パネルを共通の時間軸に揃えて**見比べることもできます（→[Align モード](#align-モード)）。
...
| ボタン | 動作 |
|---|---|
| `◐` | 黒基調 / 白基調の切り替え（選択は記憶されます） |
| `▶` | 早送り再生。直近 7 日ぶんを約 6 秒でなぞり、窓のリセットも再現します |
| `Align` | 全パネルを共通の時間軸に揃える（下記） |

### Align モード

通常は各パネルが自分の窓（`5h` なら 5 時間、`7d` なら 7 日…）を描くので、パネルどうしで横軸の意味が違います。`Align` を押すと **全パネルが同じ時間軸**（右端＝現在、幅は既定 30 日）に並び、枠をまたいだ消費の重なりを見比べられます。選択は記憶されます。

- **現在の窓だけでなく過去の窓も並べます。** 窓が切り替わるたびに値が 0 付近へ戻るので鋸歯状になります。
- **使用量が下がる区間は線も塗りも描きません。** 下降が起きるのは窓の切り替わりか窓内リセットのときだけで、連続した消費ではないためです（この扱いは通常モードでも同じです）。
- **標準ペース線（点線）は長い窓だけ**に引きます（`7d` / `1mo` / `codex 7d`）。`5h` 系は 30 日の中で本数が多すぎて読めないため出しません。窓が重なる枠（Codex は使用が再開するたびに窓を張り直します）では、その窓が現在窓だった期間にだけ描きます。
- **ヘッダは枠の名前だけ**になります（`used` / `pace` / `→reset` は現在窓の話なので出しません）。
- このモードでの `▶` は、**30 日幅の窓が 60 日前から現在まで滑る**再生になります。そのため履歴は表示幅の倍（既定 60 日）を保持しています。
- 記録が既定の長さに満たない環境では、**全ログを通じた最古の記録**まで縮みます。軸は全パネル共通なので、記録の開始が遅いパネルは左側が空になります。
